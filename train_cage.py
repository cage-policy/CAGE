...
import wandb
from model.diffusion.conditional_unet1d import (ConditionalAttnUnet1D,
                                                ConditionalUnet1D)
from model.perceiver import CausalObservationPerceiver
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='postfix of config file used for training'
    )
    parser.add_argument(
        '--seed',
        type=int,
...
# print model info, learnable parameters, non-learnable parameters, total parameters, model size, all in million
def print_model_info(model):
    ...
    for p in model.parameters():
        if p.requires_grad:
            ...
            nlp.append(p.numel())
        s.append(p.numel() * p.element_size())
    
    logger.info(f"learnable parameters: {sum(lp) / 1e6 :.2f}M")
    logger.info(f"non-learnable parameters: {sum(nlp) / 1e6 :.2f}M")
    logger.info(f"total parameters: {(sum(lp)+sum(nlp)) / 1e6 :.2f}M")
    logger.info(f"model size: {sum(s) / 1024 / 1024 :.2f}MB")
...
def initialize_model(conf):
    ...
    if conf.model.image_encoder.name.startswith('resnet'):
        Wrapper = ResNetEncoderWrapper
    elif conf.model.image_encoder.name.startswith('dino'):
        Wrapper = DinoV2EncoderWrapper
    else:
        raise NotImplementedError(f"Image encoder {conf.model.image_encoder.name} not supported")
    Wrapper = partial(
        Wrapper,
        pooled = conf.model.image_encoder.pooled,
        out_dim = conf.model.obs_dim,
    )
    if conf.model.use_in_hand and conf.model.use_fixed:
        i_encoder_2 = copy.deepcopy(i_encoder)
    
        obs_encoders = nn.ModuleDict({
            'fixed_obs': Wrapper(i_encoder),
            'in_hand_obs': Wrapper(i_encoder_2),
        })
    elif conf.model.use_in_hand:
        obs_encoders = nn.ModuleDict({
            'in_hand_obs': Wrapper(i_encoder),
        })
    else:
        obs_encoders = nn.ModuleDict({
            'fixed_obs': Wrapper(i_encoder),
        })
    if conf.model.image_encoder.use_lora:
        config = LoraConfig(
            r              = conf.model.image_encoder.lora.r,
            lora_alpha     = conf.model.image_encoder.lora.alpha,
            target_modules = list(conf.model.image_encoder.lora.target_modules),
            lora_dropout   = conf.model.image_encoder.lora.dropout,
            bias           = conf.model.image_encoder.lora.bias,
            use_rslora     = True,
        )
        for k in obs_encoders:
            obs_encoders[k].model = get_peft_model(obs_encoders[k].model, config)

    # Policy model
    perceiver = None
    if conf.model.use_perceiver:
        perceiver = CausalObservationPerceiver(
            dim         = conf.model.obs_dim,
            obs_dim     = conf.model.obs_dim,
            obs_horizon = conf.dataset.obs_horizon,
            layers      = conf.model.perceiver.layers,
            dropout     = conf.model.perceiver.dropout,
        )

    obs_num = conf.dataset.meta_data.fixed_views + conf.dataset.meta_data.in_hand_views
    if conf.model.name == 'unet':
        if conf.model.use_perceiver:
            global_cond_dim =  conf.dataset.obs_horizon * conf.model.obs_dim
        else:
            global_cond_dim = obs_num * conf.dataset.obs_horizon * conf.model.obs_dim
        # pure cnn-based unet (vanilla version)
        backbone = ConditionalUnet1D(
            ACTION_DIM,
            global_cond_dim = global_cond_dim,
            diffusion_step_embed_dim = conf.model.obs_dim,
            cond_predict_scale = True,
            **conf.model.params,
        )
    elif conf.model.name == 'attn_unet':
        backbone = ConditionalAttnUnet1D(
            ACTION_DIM,
            obs_dim = conf.model.obs_dim,
            **conf.model.params,
        )
    else:
        raise NotImplementedError(f"Unknown model: {conf.model.name}")
    
    model = CAGE(
        obs_encoders, perceiver, backbone,
        obs_dim     = conf.model.obs_dim,
        obs_horizon = conf.dataset.obs_horizon,
        obs_num     = obs_num,
    )
...
def main():
    ...
    if opt.resume is not None and not opt.new_run:
        ...
        postfix = '-' + opt.tag if opt.tag else ''
        project_path = os.path.join(opt.output_dir, f'cage{postfix}-{datetime.now()}'.split('.')[0][:-3].replace(':', '-'))
        base_conf = OmegaConf.load(os.path.join('configs', f'cage{postfix}.yaml'))

        cli = OmegaConf.from_dotlist(unknown)
        if 'model' in cli and 'name' in cli.model:
            base_conf.model.name = cli.model.name   # override model name first
    ...
    if not conf.model.image_encoder.freeze or conf.model.image_encoder.name.startswith('resnet'):
        conf.model.image_encoder.use_lora = False   # lora not applicable to resnet
    ...
    hps = {
        ...
        'configs': str(conf.dataset.meta_data.configs) if conf.dataset.meta_data.configs else 'all',
        'tasks': str(conf.dataset.meta_data.tasks) if conf.dataset.meta_data.tasks else 'all',

        'main_camera': conf.dataset.meta_data.camera_ids.main,
        'in_hand_cameras': str(conf.dataset.meta_data.camera_ids.in_hand) if conf.dataset.meta_data.camera_ids.in_hand else 'any',
        'fixed_cameras': str(conf.dataset.meta_data.camera_ids.fixed) if conf.dataset.meta_data.camera_ids.fixed else 'any',
        ...
        'use_perceiver': conf.model.use_perceiver,
        ...
        'i_emb_type': 'pooled' if conf.model.image_encoder.pooled else 'sequence',
        'i_freeze': conf.model.image_encoder.freeze,
        'i_lr_ratio': conf.model.image_encoder.lr_ratio if not conf.model.image_encoder.freeze or conf.model.image_encoder.use_lora else 'None',
        'i_use_lora': conf.model.image_encoder.use_lora,
    ...
    for k, v in conf.model.params.items():
    ...
    if conf.model.use_perceiver:
        hps['perceiver_layers'] = conf.model.perceiver.layers
        hps['perceiver_dropout'] = conf.model.perceiver.dropout
    
    if conf.model.image_encoder.use_lora:
        hps['i_lora_r'] = conf.model.image_encoder.lora.r
        hps['i_lora_alpha'] = conf.model.image_encoder.lora.alpha
        hps['i_lora_dropout'] = conf.model.image_encoder.lora.dropout
        hps['i_lora_bias'] = conf.model.image_encoder.lora.bias
        hps['i_lora_targets'] = str(conf.model.image_encoder.lora.target_modules)
    ...
    if conf.model.use_perceiver:
        print_model_info(model.perceiver)
    print_model_info(model.backbone)
    ...
    if accelerator.is_main_process:
        save_path = os.path.join(project_path, 'final_ckpt')
        if os.path.exists(save_path):
            shutil.rmtree(save_path)
        os.mkdir(save_path)
        ...
        st.save_file(model.state_dict(), os.path.join(save_path, 'model.safetensors'))
